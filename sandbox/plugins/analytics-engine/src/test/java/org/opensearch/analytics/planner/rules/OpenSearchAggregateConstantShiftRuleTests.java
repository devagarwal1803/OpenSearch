...
public class OpenSearchAggregateConstantShiftRuleTests extends BasePlannerRulesTests {
    ...
    public void testShiftedSumsCollapseToOneSumAndOneCount() {
        ...
        assertTrue("COUNT reads the base column or nothing (NOT NULL fold)", aggregate.getAggCallList().get(1).getArgList().size() <= 1);

        LogicalProject scanProject = (LogicalProject) aggregate.getInput();
        assertEquals(
            "only the shared base survives in the scan projection: " + scanProject.getProjects(),
            1,
    ...
    public void testGroupKeysPassThrough() {
        ...
        assertAggCalls(aggregate, SqlKind.SUM, SqlKind.COUNT);
        LogicalProject scanProject = (LogicalProject) aggregate.getInput();
        assertEquals("group key + shared base", List.of("$0", "CAST($1):BIGINT"), digests(scanProject.getProjects()));
    ...
    public void testUntouchedCallsAreRemappedNotDropped() {
        ...
        // base (the widest variant, cast back to SMALLINT); two bases -> SUM/COUNT pairs plus one MAX
        TableScan scan = scan();
        LogicalProject project = project(
            scan,
        ...
        // BIGINT base; SUM(RH) (its COUNT() dedups too): 4 accumulators for 4 outputs over two bases.
        assertAggCalls(aggregate, SqlKind.SUM, SqlKind.COUNT, SqlKind.MAX, SqlKind.SUM);
        LogicalProject scanProject = (LogicalProject) aggregate.getInput();
        assertEquals(
            "one column per base, no x ± k column",
    ...
    public void testCountMinMaxShiftsHoistOut() {
        // stats count(RW+1), min(RW+2), max(RW - 3), sum(RW+4) -> COUNT(x), MIN(x), MAX(x), SUM(x) over one base
        TableScan scan = scan();
        RexNode base = castBigint(ref(scan, WIDTH));
        LogicalProject project = project(scan, List.of(plus(base, 1), plus(base, 2), minus(base, 3), plus(base, 4)));
        LogicalAggregate input = aggregate(
            project,
            ImmutableBitSet.of(),
        ...
        LogicalProject scanProject = (LogicalProject) aggregate.getInput();
        assertEquals("all four terms read the single shared base", List.of("CAST($1):BIGINT"), digests(scanProject.getProjects()));
        assertNoArithmetic(scanProject);
    ...
    public void testMinMaxOnlyQueryIsRewritten() {
        // stats min(RW+5), max(RW+5): no SUM at all — MIN/MAX shifts alone justify the rewrite (one base column, no COUNT)
        TableScan scan = scan();
    ...
    public void testNullableColumnKeepsCountOfTheColumn() {
        ...
        assertEquals("COUNT must read the (nullable) base column, not fold to COUNT()", List.of(0), count.getArgList());
        assertTrue("the base column stays nullable", aggregate.getInput().getRowType().getFieldList().get(0).getType().isNullable());
        assertEquals(List.of("CAST($1):BIGINT"), digests(((LogicalProject) aggregate.getInput()).getProjects()));
    ...
    public void testNonParticipatingCallWithArgumentIsRemappedAfterPruning() {
        ...
        assertEquals(
            "shifted sums read the shared BIGINT base",
            "CAST($1):BIGINT",
    ...
    public void testMinMaxOverNarrowBaseWithHugeShiftAreNotShifted() {
    ...
    public void testNonDeterministicBaseIsNotRewritten() {
